...
void gowin_fpga_config(void)
{
    ...
    if(data != 0x900281B) {
        LOG_I("Error! Invalid device ID %X\r\n", data);
        return;
    } else {
        LOG_I("Found device ID %X\r\n", data);
    }
    ...
    LOG_I("Gowin fpga configuration done\r\n");
    ...
    if (data != 0x1F020) {
        LOG_I("Error! Bit stream download failed %X\r\n", data);
    } else {
        LOG_I("Bit stream download successfully\r\n");
    }
#endif
}
